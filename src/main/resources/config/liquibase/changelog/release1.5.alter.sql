...
CREATE TABLE IF NOT EXISTS `alert_definition` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `ux_alert_definition_statement_id` (`statement_id` ASC) VISIBLE,
  UNIQUE INDEX `ux_alert_definition_schedular_id` (`schedular_id` ASC) VISIBLE,
  CONSTRAINT `fk_alert_definition_schedular_id`
    FOREIGN KEY (`schedular_id`)
    REFERENCES `alerting`.`schedular` (`id`),
  CONSTRAINT `fk_alert_definition_statement_id`
    FOREIGN KEY (`statement_id`)
    REFERENCES `alerting`.`statement` (`id`))
ENGINE = InnoDB
AUTO_INCREMENT = 11
