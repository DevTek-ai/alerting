...
--changeset mirza:release2.2.graphs.sql
...
select id,count(category) as count,Month(created_date) as month_value,
...
case when category=1 then 'info'
when category = 2  then 'critical'
when category = 0 then 'warning'
